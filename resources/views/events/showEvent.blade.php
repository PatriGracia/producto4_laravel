...
<div class="col-auto d-flex">
    ...
    <div class="col-10 offset-md-1">
        <div id="Evento" style="margin-bottom:1em; height:500px; border: 1px solid #000; overflow: auto; padding: 1em; margin-top: 70px">
            ...
            <table class="table table-bordered table-striped" style="margin-top: 20px">
                <thead>
                    <tr>
                        ...
                        @auth
                            @if(Auth::user()->Id_tipo_usuario == 1 && $acto->Fecha < now() || Auth::user()->Id_tipo_usuario == 3 && $acto->Fecha < now())
                                <th>Subir archivo de documentación</th>
                            @endif
                        @endauth
                    </tr>
                </thead>
                <tbody>
                            ...
                            @if(Auth::user()->Id_tipo_usuario == 1 && $acto->Fecha < now() || Auth::user()->Id_tipo_usuario == 3 && $acto->Fecha < now())
                                <td>
                                    <form method="POST" action="{{route('subir')}}" accept-charset="UTF-8" enctype="multipart/form-data">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-6 mb-4">
                                                <label for="orden">Posición del documento:</label>
                                                <input type="number" class="form-control" id="orden" name="orden" required>
                                            </div>
                                        <div class="form-row">
                                            <div class="form-group col-md-6 mb-4">
                                                <input class="btn btn-success" type="submit" value="Enviar" style="margin-top: 30px">
                                            </div>
                                        </div>
                                    </form>
                                </td>
                    ...
                    <tr>
                        ...
                        @if ($datoIns == null && $acto->Fecha > now()) 
            ...
            @if(Auth::check() && $acto->Fecha < now() && $datoIns != null)
                <table class="table table-bordered table-striped" style="margin-top: 20px">
                    <thead>
                        <tr>
                            <th>#Presentación</th>
                            <th>#Acto</th>
                            <th>Localización del documento</th>
                            <th>Orden</th>
                            <th>#Persona</th>
                    ...
                    <tbody>
                        @foreach($documents as $document)
                            <tr>
                                ...
                                <td onclick="bigger('{{ ASSET('uploading/'.$document->Localizacion_documentacion) }}')" style="cursor:pointer  ; ">{{ $document->Titulo_documento }}
                                    <embed width="100px"  height="100px" src="{{ ASSET('uploading/'.$document->Localizacion_documentacion) }}" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            @auth
                @if(Auth::user()->Id_tipo_usuario == 1)
                    @csrf
                    <button type="submit" class="btn btn-info" data-toggle="modal" data-target="#editarActo">Editar Acto
                    ...
                    @csrf
                    <button type="submit" class="btn btn-danger" data-toggle="modal" data-target="#deleteActo">Eliminar Acto
